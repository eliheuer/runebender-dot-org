...
2. Inspect first. Report OS, architecture, available disk, whether Rust/Cargo is installed, and whether `runebender-xilem`, `runebender-core`, and `font-ml` are already available.
...
3. Install missing native tools with Cargo stable: `runebender-xilem`, `runebender-core`, and `font-ml`, built with `metal` on macOS, `cuda` only with a working NVIDIA toolkit, otherwise CPU-only.
