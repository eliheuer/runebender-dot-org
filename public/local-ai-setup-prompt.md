...
Set up a local Runebender workshop environment with `runebender`, `font-ml`, and verified local model files. Use the common default model root `~/.runebender/models`. Leave the user's real font unchanged.
...
2. Inspect first. Report OS, architecture, available disk, whether Rust/Cargo is installed, and whether `runebender` and `font-ml` are already available.
...
3. Install missing native tools with Cargo stable: `runebender` and `font-ml`, built with `metal` on macOS, `cuda` only with a working NVIDIA toolkit, otherwise CPU-only.
